...
result = spark.sql(f"""Delete From criminal_gold.Fact_taxa_evolucao_mensal_municipio
                        Where 1=1
...
query = spark.sql(f'''
                    ...
                    From criminal_silver.Tb_taxa_evolucao_mensal_municipio
                        Where 1=1
                          and CONCAT(ano, LPAD(mes, 2, '0')) >= '{min_yearmonth}' 
                          and CONCAT(ano, LPAD(mes, 2, '0')) <= '{max_yearmonth}'
                ''')
...
path_to_write = '/mnt/gold/criminal/Fact_taxa_evolucao_mensal_municipio'
query.write.mode("overwrite").option("mergeSchema", "true").option("path", path_to_write).saveAsTable(f"criminal_gold.Fact_taxa_evolucao_mensal_municipio")  
